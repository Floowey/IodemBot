...
namespace IodemBot.Modules.GoldenSunMechanics
{
    ...
    public class DjinnCommands : ModuleBase<SocketCommandContext>
    {
        ...
        {
            if (DjinnAndSummonsDatabase.TryGetDjinn(djinnName, out var djinn))
            {
        ...
        {
            if (DjinnAndSummonsDatabase.TryGetSummon(summonName, out var summon))
            {
                var acc = EntityConverter.ConvertUser(user ?? Context.User);
        ...
        private async Task ReleaseDjinnHidden(string djinnName)
        {
            ...
            await ReplyAsync(embed: new EmbedBuilder()
                .WithDescription(
                    $"Are you sure that you want to release your djinni {chosenDjinn.Emote} {chosenDjinn.Name}?")
                .Build());
            ...
            if (response.Content.Equals("Yes", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                _ = ReplyAsync(embed: new EmbedBuilder()
                    .WithDescription(
                        $"You set {chosenDjinn.Emote} {chosenDjinn.Name} free, who swiftly rushes off to find another master.")
                    .Build());
            }
        }
